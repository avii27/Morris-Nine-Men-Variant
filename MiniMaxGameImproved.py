...
def miniMaxGameImproved(d, gBoard, playerFlag):
    ...
    if(d == 0):
        ...
        for i in range(0, len(gBoard)):
            if(gBoard[i] == 'x'):
                if(MorrisBoard.checkMills(i, gBoard, 'W') == True):
                    mills = mills + 1
    ...
    for i in board:
        if(playerFlag == 1):
            ip = miniMaxGameImproved(d - 1, i, 0)
            ...
            ip = miniMaxGameImproved(d - 1, i, 1)
            if(ip.value < op.value):
                op.value = ip.value
                op.boardState = i
            op.count = op.count + ip.count
...
bState = getState()  
output = miniMaxGameImproved(depth, bState, 1)
